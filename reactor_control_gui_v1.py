...
class MFCGUI:
    ...
    def setup_temp_control(self, parent):
        ...
        # ttk.Label(conn_frame, text="• COM Port: COM3 (9600 baud)", foreground="blue").pack(anchor=tk.W)
    ...
    def setup_serial(self):
        ...
        try:
            ...
            self.serial_connection = serial.Serial('COM7', 38400, timeout=2, 
            ...
            messagebox.showerror("Connection Error", f"Failed to connect to COM7:\n{e}")
    ...
    def setup_alicat_serial(self):
        ...
        try:
            ...
            self.alicat_serial_connection = SerialWrapper.create('COM4', 19200)
    ...
    def connect_temp_control(self):
        ...
        try:
            ...
            # Create serial connection to COM3

            self.temp_serial = SerialWrapper.create('COM3', 9600)
...
"""
    ...
    def refresh_plot(self):
        ...
        try:
            ...
            with open(self.mfc_log_filename, 'r', encoding='utf-8', errors='ignore') as f:

                # Read content and remove null characters
                ...
                if lines and lines[0].startswith('datetime'):
                ...
                for line in lines:
                    ...
                    # Parse CSV line manually to avoid null character issues
                    ...
                    ts_str, mfc_name, pressure, temp, flow, setpoint, gas = parts[:7]
                    ...
                    # Map name to key - clean up the name first

                    key = None

                    mfc_name_clean = mfc_name.strip().strip('"').strip()

                    

                    print(f"Parsing MFC: '{mfc_name}' -> cleaned: '{mfc_name_clean}'")

                    

                    if mfc_name_clean in ("1", "N2", "N2 MFC"):

                        key = 1

                        print(f"  -> Mapped to key 1 (N2)")

                    elif mfc_name_clean in ("3", "CO2", "CO2 MFC"):

                        key = 3

                        print(f"  -> Mapped to key 3 (CO2)")

                    elif mfc_name_clean in ("6", "H2", "H2 MFC"):

                        key = 6

                        print(f"  -> Mapped to key 6 (H2)")

                    elif mfc_name_clean.lower().startswith("sampling"):

                        key = 'sampling'

                        print(f"  -> Mapped to key 'sampling'")

                    

                    if key is None:

                        print(f"  -> No mapping found for '{mfc_name_clean}', skipping")
